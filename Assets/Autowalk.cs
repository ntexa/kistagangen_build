...
public class Autowalk : MonoBehaviour
{
	...
	public double xLimitUp;
	public double xLimitDown;
    ...
    void Update()
	{
		...
		if (isWalking) {
			if (mainCamera.transform.position.x >= xLimitDown  && mainCamera.transform.position.x <= xLimitUp ) {
				...
				if (mainCamera.transform.position.x < xLimitDown) {
					Vector3 direction = new Vector3 (mainCamera.transform.forward.x + step, 0, mainCamera.transform.forward.z).normalized * speed * Time.deltaTime;
					Quaternion rotation = Quaternion.Euler (new Vector3 (0, -transform.rotation.eulerAngles.y, 0));
					transform.Translate (rotation * direction);
				} else {
					Vector3 direction = new Vector3 (mainCamera.transform.forward.x - step, 0, mainCamera.transform.forward.z).normalized * speed * Time.deltaTime;
		...
		if (freezeYPosition) {
			if (mainCamera.transform.position.x  >= xLimitDown  && mainCamera.transform.position.x <= xLimitUp ) {
				...
				if (mainCamera.transform.position.x < xLimitDown) {
					transform.position = new Vector3 (transform.position.x+step, yOffset, transform.position.z);
				} else {
					transform.position = new Vector3 (transform.position.x-step, yOffset, transform.position.z);
				}
			}
		...
		if (freezeZPosition) {
			if (mainCamera.transform.position.x >= xLimitDown  && mainCamera.transform.position.x <= xLimitUp) {
				...
				if (mainCamera.transform.position.x < xLimitDown) {
					transform.position = new Vector3 (transform.position.x+step, yOffset, 0);
				} else {
					transform.position = new Vector3 (transform.position.x-step, yOffset, 0);
				}
			}
